    ...
    async function loadSearchcraftSDK() {
        try {
            ...
            if (module.Searchcraft) {
                ...
                const searchcraft = new module.Searchcraft(config);
                ...
                console.warn('Searchcraft: Searchcraft class not found in module');
                showConfigurationError('Failed to initialize Searchcraft SDK.');
            }

        } catch (error) {
            console.error('Searchcraft: Failed to load SDK', error);
            showConfigurationError('Failed to load Searchcraft SDK. Please check your internet connection.');
        }
    }

    /**
     * Get the Searchcraft SDK URL
     */
    function getSDKUrl() {
        // TODO We could make the version selectable from the admin settings here
        //
        // if (typeof searchcraft_config !== 'undefined' && searchcraft_config.sdkUrl) {
        //     return searchcraft_config.sdkUrl;
        // }

        const version = '0.11.1';
        return `https://cdn.jsdelivr.net/npm/@searchcraft/javascript-sdk@${version}/dist/components/index.js`;
    }
